    ...
    NgValidations.prototype.handleConditionalValidations = function (control, form) {
        ...
        changes.subscribe(function (value) {
            conditions.forEach(function (condition, index) {
                ...
                var found;
                condition.values.forEach(function (exp) {
                    exp.flags = exp.flags || '';
                    var reg = new RegExp(exp.pattern, exp.flags);
                    if (reg.test(value))
                        found = true;
                });
                if (currentControl && found && conditionsToValidate.indexOf(condition) < 0) {
                    ...
                    conditionsToValidate = conditionsToValidate.filter(function (c) { return JSON.stringify(c) !== JSON.stringify(condition); });
                }
                controlRequired = conditionsToValidate.findIndex(function (formControl) {
                    return formControl.required === true;
                }) >= 0;
                conditionalValidators = conditionsToValidate.filter(function (c) { return c.control === condition.control; }).map(function (c) { return c.tests.map(_this.mapValidators); }).reduce(function (a, b) { return a.concat(b); }, []);
